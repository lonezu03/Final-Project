...
public class HistoryDepositService {
	...
	public List<HistoryDepositGetAllRespone> getAllHistoryDepositB() {
		return historyDepositRepository.findAll().stream().map(t -> {
			HistoryDepositGetAllRespone respone = historyDepositMapper.toHistoryDepositGetAllRespone(t);
			respone.setAvatarUser(t.getUser().getAvatarUser());
			respone.setEmailUser(t.getUser().getEmailUser());
			respone.setRole(t.getUser().getRole());
			respone.setUserNameUser(t.getUser().getUserNameUser());
			return respone;
	...
	List<HistoryDepositRespone> getAllHistoryDepositByUser(String idUser) {
		...
		return historyDepositRepository.findByUser(user).stream()
				.map(t -> historyDepositMapper.toHistoryDepositRespone(t)).collect(Collectors.toList());
	}
	...
	public HistoryDepositRespone updateHistoryDeposit(String idHistoryDeposit, StatusDeposit statusDeposit) {
		HistoryDeposit historyDeposit = historyDepositRepository.findById(idHistoryDeposit)
				.orElseThrow(() -> new AppException(ErrorCode.DEPOSIT_NOT_EXISTS));
		...
		if (statusDeposit == StatusDeposit.SUCCESS) {
			User user = userRepository.findById(historyDeposit.getUser().getIdUser())
					.orElseThrow(() -> new AppException(ErrorCode.USER_NOT_EXISTED));
